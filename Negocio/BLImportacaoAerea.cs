    ...
    public class BLImportacaoAerea
    {
        ...
        public async Task<HouseWaybill> ConverterEntidadeEmXFZB(ImportacaoAereaEntidade entidade)
        {
            try
            {
                ...
                {
                    HouseWaybill arquivoHouse = new HouseWaybill($"{entidade.NR_HOUSE.Replace("-", "")}_{entidade.NR_MASTER.Remove(8, 1)}", "House Waybill", "703", "Creation");
                    ...
                    arquivoHouse.MessageHeaderDocument.recipientParties = new List<RecipientParty>
                {
                    ...
                    arquivoHouse.BusinessHeaderDocument.ID = entidade.NR_HOUSE.Replace("-", "");
                    ...
                    arquivoHouse.MasterConsignment.IncludedHouseConsignment.ID = entidade.NR_HOUSE.ConverterNuloParaBranco().Replace("-", "");
                    ...
                    arquivoHouse.MasterConsignment.IncludedHouseConsignment.TotalPrepaidChargeAmount.TotalPrepaidChargeAmount = entidade.ValorFretePreppaid.ToString() == "" ? "0" : entidade.ValorFretePreppaid.ToString().Replace(",", ".");
                    arquivoHouse.MasterConsignment.IncludedHouseConsignment.TotalPrepaidChargeAmount.currencyID = entidade.MoedaFretePreppaid;
                    ...
                    arquivoHouse.MasterConsignment.IncludedHouseConsignment.TotalCollectChargeAmount.currencyID = entidade.MoedaFreteCollect;
        ...
        public async Task<HouseManifest> ConverterEntidadeEmXFHL(ImportacaoAereaEntidade entidade)
        {
            try
            {
                ...
                if (entidade.NR_HOUSE != null && entidade.NR_MASTER != null)
                {
                    HouseManifest arquivoHouse = new HouseManifest($"{entidade.NR_HOUSE.Replace("-", "")}_{entidade.NR_MASTER.Remove(8, 1)}", "Cargo Manifest", "785", "Creation");
                    arquivoHouse.MessageHeaderDocument.senderParties = new List<SenderParty>
                {
                    //new SenderParty{PrimaryID = new SenderPrimaryIDXML{ PrimaryID = "RIEGESOFTWARE", schemeID = "C" } },
                    new SenderParty{PrimaryID = new SenderPrimaryIDXML{ PrimaryID = "REUAGT88FORWARDER/FRA01", schemeID = "P" } }
